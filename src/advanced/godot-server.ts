/*
 * Vendored from tugcantopaloglu/godot-mcp (MIT), which extends
 * Coding-Solo/godot-mcp (MIT). Integrated into godot-mcp-server with light
 * edits only: the standalone MCP bootstrap was removed and the tool list is
 * exposed as getToolDefinitions() + callTool() for embedding. See NOTICE.
 */
/**
 * Godot MCP Server
 *
        ...
        },
{
  ...
  async callTool(name: string, args: any): Promise<any> {
    this.logDebug(`Handling tool request: ${name}`);
    switch (name) {
        case 'launch_editor':
